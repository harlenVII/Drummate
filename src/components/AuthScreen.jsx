          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {isSignUp && (
              <input
                ...
                autoComplete="name"
                className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 text-base text-gray-800 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
              />
            )}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3 bg-violet-500 text-white font-semibold rounded-xl hover:bg-violet-600 transition-colors disabled:opacity-50"
            >
              {submitting
                ? t('auth.syncing')
                : isSignUp ? t('auth.signUp') : t('auth.signIn')
