    ...
    <div role="group" aria-label={ariaLabel || t('selectCategory')} className="inline-flex rounded-md overflow-hidden border border-gray-300 dark:border-slate-600">
      <button
        ...
        className={`px-2 py-1 text-xs font-semibold ${
          value === 'fundamentals' ? 'bg-accent-600 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-600'
        }`}
      >
      ...
      <button
        ...
        className={`px-2 py-1 text-xs font-semibold border-l border-gray-300 dark:border-slate-600 ${
          value === 'songs' ? 'bg-accent-600 text-white' : 'bg-white dark:bg-slate-700 text-gray-600 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-600'
        }`}
      >
  ...
  return (
    <div
      ...
      className={`text-sm italic px-3 py-4 rounded-lg border border-dashed transition-colors ${
        isOver ? 'bg-accent-50 border-accent-400 text-accent-600' : 'bg-white dark:bg-slate-800 border-gray-300 dark:border-slate-600 text-gray-400 dark:text-slate-500'
      }`}
    >
      {label}
    </div>
        ...
        <div className={`flex-1 flex items-center justify-between ml-2 ${item.archived ? 'opacity-50' : ''}`}>
          {editingItemId === item.id ? (
            <input
              ...
              onBlur={commitRename}
              autoFocus
              className="flex-1 mr-3 px-3 py-1 border border-accent-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-500 dark:bg-slate-700 dark:text-slate-100"
            />
          ) : (
            <span
              onClick={() => startRename(item)}
              className="font-medium text-gray-800 dark:text-slate-100 cursor-pointer hover:text-accent-600 transition-colors"
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          <input
            ...
            className="flex-1 px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-500 dark:bg-slate-700 dark:text-slate-100"
          />
          <button
            onClick={handleAdd}
            disabled={!newName.trim()}
            className="px-4 py-2 bg-accent-600 text-white rounded-lg font-medium hover:bg-accent-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            + {t('add')}
          </button>
        </div>
    ...
    return (
      <div
        ...
        className={`bg-white dark:bg-slate-800 shadow-sm flex items-center justify-between transition-colors cursor-pointer ${
          ...
          isActive ? 'ring-2 ring-accent-500' : isFocused ? 'ring-2 ring-gray-300 dark:ring-slate-600' : ''
        }`}
      >
